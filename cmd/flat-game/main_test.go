...
func TestAtMostOneBufferedTurnPerTick(t *testing.T) {
	// Two turns queued before a tick: only the last valid one applies.
	d := driver(2)
	start := headOf(d)
	...
	d.Advance(move1)
	if got := headOf(d); got != (point{start.x, start.y + 1}) {
		t.Fatalf("last buffered turn (down) should win; head = %v, want %v", got, point{start.x, start.y + 1})
	}
}
...
func TestWallCollisionEndsGame(t *testing.T) {
	...
	s.dir, s.nextDir = dirRight, dirRight
	s.step()
	if !s.over {
...
func TestSelfCollisionEndsGame(t *testing.T) {
	...
	s.dir, s.nextDir = dirRight, dirDown
...
func TestMovingIntoVacatingTailIsSafe(t *testing.T) {
	...
	s.snake = []point{{5, 5}, {5, 6}, {6, 6}, {6, 5}}
	s.dir, s.nextDir = dirUp, dirRight // {5,5} -> {6,5} (the tail)
	s.food = point{0, 0}
	s.step()
	if s.over {
