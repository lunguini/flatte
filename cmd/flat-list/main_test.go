...
func TestClickSelectsRowAccountingForScroll(t *testing.T) {
	...
	Handle(s, flat.MouseEvent{Button: flat.MouseLeft, Action: flat.MousePress, Y: 4}, flat.Effects[State]{})
	if s.list.Cursor() != 4 {
...
func TestViewKeepsChromePinned(t *testing.T) {
	s := ready()
	Handle(s, key('G'), flat.Effects[State]{}) // scroll to bottom
...
func TestViewInitialSnapshot(t *testing.T) {
	s := NewState()
	s.layout(24)
	flatest.AssertGoldenFrame(t, "testdata/list.golden", View(s, flat.RenderContext{Width: 72}))
}
