...
func TestCursorMovementBackspaceAndDeleteAreFieldLocal(t *testing.T) {
	...
	Handle(state, flatte.KeyEvent{Key: flatte.KeyLeft}, flatte.Effects[State]{})
	Handle(state, flatte.KeyEvent{Key: flatte.KeyBackspace}, flatte.Effects[State]{})
	Handle(state, flatte.KeyEvent{Key: flatte.KeyDelete}, flatte.Effects[State]{})
...
func TestModifiedArrowsMoveFocusedFieldByWord(t *testing.T) {
	...
	Handle(state, flatte.KeyEvent{Key: flatte.KeyLeft, Mod: flatte.ModAlt}, flatte.Effects[State]{})
	if state.fields[0].Input.Cursor != len("hello ") {
		t.Fatalf("alt-left cursor = %d, want start of world", state.fields[0].Input.Cursor)
	}
	Handle(state, flatte.KeyEvent{Key: flatte.KeyLeft, Mod: flatte.ModCtrl}, flatte.Effects[State]{})
	if state.fields[0].Input.Cursor != 0 {
		t.Fatalf("ctrl-left cursor = %d, want start", state.fields[0].Input.Cursor)
	}
	Handle(state, flatte.KeyEvent{Key: flatte.KeyRight, Mod: flatte.ModCtrl}, flatte.Effects[State]{})
	if state.fields[0].Input.Cursor != len("hello") {
...
func TestEscapeBlursAndQQuitsOnlyWhenBlurred(t *testing.T) {
	...
	Handle(state, flatte.KeyEvent{Key: flatte.KeyEscape}, fx)
	Handle(state, flatte.KeyEvent{Key: flatte.KeyCharacter, Rune: 'q'}, fx)
	if !quit {
...
func TestViewPlacesCursorInFocusedField(t *testing.T) {
	...
	state.focused = 1
	if second := View(state, flatte.RenderContext{Width: 72}); second.Cursor == nil || second.Cursor.Y != 5 {
...
func TestViewMatchesSubmittedSnapshot(t *testing.T) {
	...
	flatest.AssertGoldenFrame(t, "testdata/submitted.golden", View(state, flatte.RenderContext{Width: 72}))
}
