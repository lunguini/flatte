...
func adderApp() flatcore.App[adder] {
	return flatcore.App[adder]{
		State: &adder{},
		Handle: func(s *adder, ev flatcore.Event, fx flatcore.Effects[adder]) {
		...
		View: func(s *adder, ctx flatcore.RenderContext) flatcore.Frame {
			return flatcore.Frame{Content: "n"}
		},
	}
}
...
func TestRecorderCapturesEventAndUpdateStream(t *testing.T) {
	...
	for _, s := range rec.Steps {
		if k, ok := s.Event.(flatcore.KeyEvent); ok && k.Rune == '+' {
			sawKey = true
		}
	}
	if !sawKey {
