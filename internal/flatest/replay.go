...
import "github.com/lunguini/flat/internal/flatcore"

// Step is one entry in a recorded session: either an event the loop
// received or an update that was applied.
type Step struct {
	Event  flatcore.Event // non-nil for an event step
	Update string         // non-empty for an update step
}

// Recorder is a flatcore.Tracer that captures the ordered event/update
// stream of a session for replay assertions. Use it as App.Tracer.
type Recorder struct {
	Steps []Step
}

func (r *Recorder) Event(ev flatcore.Event) { r.Steps = append(r.Steps, Step{Event: ev}) }
...
func (r *Recorder) Events() []flatcore.Event {
	var events []flatcore.Event
	for _, s := range r.Steps {
		if s.Event != nil {
...
func Replay[S any](app flatcore.App[S], width int, rec *Recorder) *Recorder {
	out := &Recorder{}
	app.Tracer = out
	d := Start(app, width)
	for _, ev := range rec.Events() {
		if _, isResize := ev.(flatcore.ResizeEvent); isResize {
			continue
		}
		d.Send(ev)
		d.Settle()
